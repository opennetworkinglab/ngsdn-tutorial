...
We will start with a simple YANG module called `demo-port` in [`yang/demo-port.yang`](./yang/demo-port.yang)
...
*Extra Credit:* If you have some time, take a look at the models in the `/models` directory or browse them on Github: https://github.com/openconfig/public/tree/master/release/models
...
This file contains a top-level Ports message that matches the structure defined in the YANG model. You can see that `proto_generator` also adds a `yext.schemapath` to each leaf that explicitly maps the member the YANG leaf. Enums (like `tutorial.enums.DemoPortSPEED`) aren't included in this file, but `proto_generator` puts them in a separate file: `enums.proto`
...
*Extra Credit:* Try to find the members used to enable a port or get the ingress packets counter in the protobuf messages.

*Hint:* Searching by schemapath might help.

------

`ygot` can also be used to generate Go structs that adhere to the YANG model that are capable of validating the structure, type, and values of data.
...
The value is very human readable, but we can translate the reply using a utility that converts between the binary and textual representations of the protobuf message.

We can rerun the command, but this time pipe the output throught the converter utility (then pipe that output to `less` to make scrolling easier):

```
$ util/oc-pb-decoder --grpc-addr localhost:50001 get /
```

The contents of the response should be easier to read. Scroll down to the first `interface`. Is the interface enabled? What is the speed of the port?

*Extra credit:* Can you find `in-pkts`? If not, why do you think they are missing?
...
We have the subscription path, the type of subscription (sampling) and the sampling rate (every 1000ms, or 1s).
...
Each response has a timestamp, path, and new value. Because we are sampling, you should a new update printed every second. Leave this running, while we generate some traffic.
...
You should see replies printing in the Mininet CLI.
...
In the gNMI set window, you should see request indicating the new value for the `enabled` leaf:
```
REQUEST
update {
  path {
...
In the gNMI subscription window, you should see a new response indicating that the operational status of `leaf1-eth3` is `DOWN`:
```
RESPONSE
update {
  timestamp: 1567896891549363399
